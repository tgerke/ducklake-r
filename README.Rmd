...
Each layer is automatically versioned, providing complete data lineage from raw source through to analysis-ready datasets. Each layer can live in a schema of its own (`create_schema()`), which keeps `bronze.vehicles`, `silver.vehicles`, and `gold.vehicle_efficiency` one lineage under three roofs, and lets access to raw data be restricted at the schema level. This approach enables:
...
dplyneage's [ducklake lineage vignette](https://tgerke.github.io/dplyneage/articles/ducklake-lineage.html) walks through a full example, including per-layer diagrams for medallion pipelines and lineage for time-travel queries.
