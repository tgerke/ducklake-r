...
and their `reset_` counterparts. DuckLake shows pending keys nowhere, so
ducklake notes them as those functions run: keys changed in the same
transaction with a raw `ALTER TABLE` statement are not seen, and the
rewrite then carries the committed keys over instead.
