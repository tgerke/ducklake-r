...
- **Silver Layer (Cleaned)**: Standardized and cleaned data: SDTM blanks converted to `NA`, type conversions, validation. This is the trusted source for deriving analysis datasets.
...
character variables use the empty string for missing values, and R analysis
code expects `NA`. admiral's `convert_blanks_to_na()` does that on a data
frame. On a lazy lake table it has no effect, because nothing has been read
...
1. **Every table creation is versioned** - Both the bronze (`dm_raw`) and silver (`dm`) layers have their own snapshot
...
Every committed change to a DuckLake table creates a versioned snapshot, so
the audit trail is complete whichever function you use. The choice is about
the *kind* of change (see `vignette("modifying-tables")` for the full
guidance):

- **New derived variables**: declare them with `add_table_column()` and fill
  them with a `mutate()` pipeline through `ducklake_exec()`, as in the
  example above. The column is a metadata change and the fill is an
  in-database UPDATE; nothing is collected into R.
- **Targeted corrections** (fixing a flag for one subject, appending a
  handful of records): use `rows_update()`, `rows_insert()`, or
  `rows_delete()`. The change runs as a single SQL statement without
  rewriting the rest of the table, and small changesets benefit from
  DuckLake's data inlining.
- **Bulk reshapes** (recomputing or filtering most of a dataset, changing
  its grain): use `replace_table()` wrapped in `with_transaction()`. It
  rewrites the whole table, which is the right tool when most rows change
  anyway, and it carries the table's labels, partition keys, sort order,
  and options over to the rewritten table.
...
Inlining benefits clinical workflows in a specific way: when a statistical
programmer corrects a single subject's flag or updates a derived variable for a
handful of records, the change is captured atomically in the catalog without
creating a proliferation of tiny Parquet files.

Before archival or regulatory submission, flush all inlined data to Parquet so
the data path contains a fully self-describing, portable set of files:

```{r pre-archival-checkpoint, eval = FALSE}
# Materialise any inlined data to Parquet before archival
flush_inlined_data()
```

This ensures that the archived lake can be read by any Parquet-capable tool, and
aligns with ICH E6(R2) and 21 CFR Part 11 expectations for long-term data
retention. For more details see `vignette("data-inlining")`.
...
7. **Functionality**: Demonstrated versioning, time travel, transactions, and record updates
...
- `vignette("data-inlining")` - Data inlining and storage optimisation
